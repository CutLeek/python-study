...
for i in range(len(listaaaa)):
    for j in range(len(listaaaa) - 1):
        if listaaaa[j + 1] < listaaaa[j]:
            listaaaa[j], listaaaa[j + 1] = listaaaa[j + 1], listaaaa[j]
print(listaaaa)

# 列表推导式：
names = ['tom', 'lily', 'bob', 'tomas', 'jack', 'heals']
result1 = [name for name in names if len(name) > 3]
print(result1)

result2 = [x.capitalize() for x in names]
print(result2)

result3 = [(i, j) for i in range(5) if i % 2 == 0 for j in range(5) if j % 2 != 0]
print(result3)
